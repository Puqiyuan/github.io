...
Table）中选取描述符，这些描述符实际就是一个结构体，描述有一个段的基地址、限长以及访问权限等。而逻辑地址的低一些位用来在在段内进行偏移。

Linux内核在cpu_init_exception_handling函数里先通过set_tss_desc设置tss描述符，然后通过调用load_TR_desc将tss描述符对应的段选择子加载到task register（TR）里，
