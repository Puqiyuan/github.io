...
#+end_example
.data..percpu这个section的Type是PROGBITS，Flags是WA，意味着其在vmlinux镜像里是占有物理空间的，bootloader加载内核并解压时，会复制这部分内容到物理内存，这样一份模板的percpu空间以这种方式就开好了，当内核运行起来时通过setup_per_cpu_areas函数会为每个cpu按这个模板生成各自的percpu空间，并且设置各自的percpu offset，当然里面实现逻辑就会利用__per_cpu_start这些符号去获取percpu空间的大小。

还有一个问题，就是cea里的虚拟地址又是怎么来的。

然后分析Linux对tss描述符的设置与设置到硬件，Linux内核在cpu_init_exception_handling函数里先通过set_tss_desc设置tss段的描述符，然后通过调用load_TR_desc将tss描述符对应的段选择子加载到task register（TR），关于这点（个流程）后续还会从代码上去说明。
