...
cea_map_percpu_pages还是通过调用cea_set_pte函数来完成内核虚拟地址到物理percpu page的页表映射。cpu_tss_rw就是percpu的变量：
#+begin_example
/*
...
#+end_example
这和前面的gdt_page类似，也是利用了percpu机制。但是注意这里sp0并没有设置为有效的值，因为sp0在从较低的权限级进入较高的ring 0时才被使用，而init task就是运行内核代码的处于ring 0，不存在这个场景，而普通进程在何时设置成什么值，后面再分析。TSS的概念是per-cpu的，因为在Linux上进程切换主要由软件来完成，和硬件提供的tss结构体没有关系（不使用硬件机制进行进程切换）。

继续往下分析set_tss_desc函数：
#+begin_example
arch/x86/include/asm/desc.h
...
#+end_example
set_tss_desc接受两个参数，cpu是指当前要设置哪个cpu的tss描述符，而addr是tss段的地址（而不是tss描述符），这里就是percpu的变量cpu_tss_rw，类型为tss_struct，这个地址会参与构建tss描述符，主要就是通过set_tssldt_descriptor设置tss描述符里的几个base部分，也就是cpu_tss_rw的地址被拆成几个部分写到ldttss_desc里的base0-3：
#+begin_example
...
#+end_example
为9的类型在intel sdm vol3里是有明确类型的，所有类型的表包括如下图：
#+CAPTION: 系统段以及门描述符类型
#+LABEL: fig:system-segment-gate-desc-type
#+ATTR_HTML: alt="" title="" align="center" :width 20% :height 20%
[[./img/system-segment-gate-desc-type.png]]

为9可以是IA-32e模式下的64位tss desc描述符类型，所有系统段描述符S位都是0，memset在一开始清0了整个描述符。

最后的write_gdt_entry其实就是个memcpy了：
#+begin_example
...
#+end_example
这里虽然最终改动是对内存的，但是由于先前将这个gdt_page设置到了gdtr寄存器，所以改动gdt_page里的描述符对硬件就会立即生效。

注意以上逻辑先通过get_cpu_gdt_rw获得当前cpu的gdt_page，然后才能使用里面的第GDT_ENTRY_TSS个条目作为tss描述符，这也是为什么前面有一些篇幅执着于分析清楚gdt的生效/设置逻辑，x86-64下，GDT_ENTRY_TSS按如下定义：
#+begin_example
/* Needs two entries */
#define GDT_ENTRY_TSS			8
#+end_example

到目前费了半天劲就分析清楚了gdt以及tss描述符在Linux下生效的实现。但是tss段本身的内容如前有图展示的，比如rsp0怎么设置的，设置了什么这些又是另外的问题需要分析了，前面也提到过这个问题，下面分析清楚这些问题。

其实前面提到过，tss在Linux上并不是per-task的概念，而是per-cpu的，这点尤其在设置tss里的sp0得到明确体现：
#+begin_example
arch/x86/kernel/cpu/common.c
...
#+end_example
注释说的很清楚，cpu_init就是初始化per-CPU的数据，这包括sp0，也就是说不论当前跑的是哪个用户进程，当其需要进入ring 0的内核代码执行时，都是用的这里设置的针对这个cpu的sp0。load_sp0最终写的就是percpu的x86_tss里的sp0：
#+begin_example
arch/x86/include/asm/processor.h
...
前面分析过x86_tss的地址已经设置到gdt_page里对应的描述符了。但是具体的sp0的值要从cpu_entry_stack的实现分析：
#+begin_example
arch/x86/include/asm/cpu_entry_area.h
...
#+end_example
可见，与上面的cpu_entry_area里的量类似，entry_stack_page成员需要percpu的物理页：
#+begin_example
arch/x86/mm/cpu_entry_area.c

static DEFINE_PER_CPU_PAGE_ALIGNED(struct entry_stack_page, entry_stack_storage);
#+end_example
建立映射依旧是在setup_cpu_entry_area函数里了：
#+begin_example
arch/x86/mm/cpu_entry_area.c
...
以上就终于分析完了问题：[当在用户态触发中断时，“CPU会自动切换到TSS.IST或TSS.sp0指定的栈顶”，那么这个栈顶是怎么设置并起效的呢？]。后面可以继续中断流程的代码分析了，下面继续分析函数call_on_irqstack_cond：
#+begin_example
/*
 * Macro to invoke system vector and device interrupt C handlers.
 */
