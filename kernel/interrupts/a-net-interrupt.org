...
#+end_example
以上cea_set_pte之前介绍过，总结下，cpu_entry_area结构体只是描述了一个内存布局，但是内存本身一般没有alloc_*这种接口来分配，而是一般来自percpu的空间，然后通过cea_set_pte手工设置页表，来完成cpu_entry_area里的虚拟地址到物理页面的映射。另外一个题外话，percpu的空间又是怎么开出来的呢？
