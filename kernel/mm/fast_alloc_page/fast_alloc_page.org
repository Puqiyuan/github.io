...
#+end_example
从这个函数可以看出填充zonerefs的一个顺序：就是按照node_order里指明的节点依次取出node作为参数，调用build_zonerefs_node，这样自然的效果就是距离当前处理节点（需要填充当前节点的node_zonelists）越近的节点，其里面的zone(s)排在zoneref（zonelist：_zonerefs）数组元素的前面。

这里再稍微看下build_zonerefs_node函数的实现：
#+begin_example
...
这里需要详细理解这一点，因为[[ac-preferred_zoneref][后面的]]分配需要确定alloc_context: preferred_zoneref。
