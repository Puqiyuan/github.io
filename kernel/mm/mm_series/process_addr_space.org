...
page fault主要有两种类型，一是major的，再者是minor的．major fault就是说数据必须从磁盘上读，这是十分昂贵的操作．而minor fault是页面在内存里但可能不属于当前进程．Linux将这个两个信息维护在task_struct->maj_flt和task_struct->min_flt成员里．
* 拷贝到/拷贝自用户空间
